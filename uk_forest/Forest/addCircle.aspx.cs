...
{
    public partial class addCircle : System.Web.UI.Page
    {
        string token_sess;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    bind_zone_name();
                    grid_name_circle(Convert.ToInt32(ddl_zone_list.SelectedValue));
        ...
        public string add_zone(string zone_id, string circle_name, string created_by)
        {
            try
            {
                HttpClient client = new HttpClient();
                ...
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token_sess);
                var url = apiUrl + "TblCircleMasters/PostTblCircleMaster";
        ...
        protected void btn_add_circle_Click(object sender, EventArgs e)
        {
            try
            {
                string result = add_zone(ddl_zone_list.SelectedValue, txt_circle_name.Text, Session["UserId"]?.ToString());
                txt_circle_name.Text = "";
                ...
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Failed to add Circle. Please try again.');", true);
                }
                ClearControls();
                grid_name_circle(Convert.ToInt32(ddl_zone_list.SelectedValue));
            }
        ...
        void grid_name_circle(int zoneId)
        {
            try
            {
                using (var client = new HttpClient())
                {
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token_sess);
                    HttpResponseMessage Res = client.GetAsync(apiUrl + ("TblCircleMasters/GetTblCircleMasterByZoneId?zone_id=" + zoneId)).Result;
                    if (Res.IsSuccessStatusCode)
                    {
                        var EmpResponse = Res.Content.ReadAsStringAsync().Result;
                        DataTable dt = (DataTable)JsonConvert.DeserializeObject(EmpResponse, (typeof(DataTable)));
                        grid_circle_names.DataSource = dt;
                        grid_circle_names.DataBind();
                    }
                }
            }
            catch (Exception ex)
        ...
        protected void grid_circle_names_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            try
            {
                grid_circle_names.PageIndex = e.NewPageIndex;
                this.grid_name_circle(Convert.ToInt32(ddl_zone_list.SelectedValue));
            }
            catch (Exception ex)
            {
