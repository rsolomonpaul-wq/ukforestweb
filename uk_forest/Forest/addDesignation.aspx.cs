...
namespace uk_forest.Forest
{
    public partial class addDesignation : System.Web.UI.Page
    {
        ...
        public string add_designation(string designationName, string CreatedBy)
        {
            try
            {
                var data = new
                ...
                var url = apiUrl + "TblDesignationMasters/PostTblDesignationMaster";
        ...
        protected void btn_add_designation_Click(object sender, EventArgs e)
        {
            try
            {
                string result = add_designation(txt_designation_name.Text, Session["designationId"]?.ToString());
                ...
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "alert", "alert('Failed to add designation. Please try again.');", true);
                }
                ClearControls();
                grid_name_designation();
            ...
            {
                Console.WriteLine($"Error during designation addition: {ex.Message}");
        ...
        void grid_name_designation()
        {
            try
            {
                using (var clinet = new HttpClient())
                {
                    ...
                    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token_sess);
                    HttpResponseMessage Res = clinet.GetAsync(apiUrl + ("TblDesignationMasters/GetTblDesignationMasters")).Result;
                    if (Res.IsSuccessStatusCode)
                    {
                        ...
                        grid_designation_names.DataSource = dt;
                        grid_designation_names.DataBind();
                    }
                }
            }
            catch (Exception ex)
        ...
        protected void grid_designation_names_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            try
            {
                grid_designation_names.PageIndex = e.NewPageIndex;
                this.grid_name_designation();
            }
            catch (Exception ex)
            {
                throw ex;
