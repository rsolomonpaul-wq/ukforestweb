...
namespace uk_forest.Forest
{
    public partial class addRole : System.Web.UI.Page
    {
        ...
        public string add_role(string RoleName, string CreatedBy)
        {
            try
            {
                var data = new
                ...
                var url = apiUrl + "TblRolesMasters/PostTblRolesMaster";
        ...
        protected void btn_add_role_Click(object sender, EventArgs e)
        {
            try
            {
                string result = add_role(txt_role_name.Text, Session["RoleId"]?.ToString());
        ...
        void grid_name_role()
        {
            try
            {
                using (var clinet = new HttpClient())
                {
                    ...
                    client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", token_sess);
                    HttpResponseMessage Res = clinet.GetAsync(apiUrl + ("TblRolesMasters/GetTblRolesMasters")).Result;
                    if (Res.IsSuccessStatusCode)
                    {
                        ...
                        grid_role_names.DataSource = dt;
                        grid_role_names.DataBind();
        ...
        protected void grid_role_names_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            try
            {
                grid_role_names.PageIndex = e.NewPageIndex;
                this.grid_name_role();
            }
            catch (Exception ex)
            {
